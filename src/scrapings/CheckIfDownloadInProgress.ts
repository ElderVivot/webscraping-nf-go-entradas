...
async function downloadInProgress (page: Page): Promise<string> {
    return new Promise(resolve => {
        ...
        const interval = setInterval(async () => {
            ...
            logger.debug(`- Processando a ${quantityTimesCheckIfDownloadInProgress * 5} segundos`)
...
export async function CheckIfDownloadInProgress (page: Page, settings: ISettingsNotes): Promise<void> {
    try {
